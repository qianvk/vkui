...
namespace QWK {
    ...
    void QtWindowContext::winIdChanged(WId winId, WId oldWinId) {
        ...
        if (!m_windowHandle || m_applyingFramelessFlag) {
            return;
        }

        // Changing QWidget window flags recreates its platform window and emits
        // another WinIdChange. Avoid reapplying an identical flag set, which
        // otherwise recurses indefinitely on headless and portable QPA plugins.
        const Qt::WindowFlags flags = m_delegate->getWindowFlags(m_host);
        if (!flags.testFlag(Qt::FramelessWindowHint)) {
            const QScopedValueRollback applyingFlag(m_applyingFramelessFlag, true);
            m_delegate->setWindowFlags(m_host, flags | Qt::FramelessWindowHint);
        }
    }
