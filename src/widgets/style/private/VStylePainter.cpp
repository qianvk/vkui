...
QColor VStylePainter::mix(const QColor& from, const QColor& to, qreal progress) {
...
QColor VStylePainter::multiplyAlpha(const QColor& color, qreal factor) {
    QColor result = color;
    result.setAlphaF(static_cast<float>(std::clamp(color.alphaF() * factor, 0.0, 1.0)));
    return result;
}

QColor VStylePainter::contrastingText(const QColor& background) {
...
QColor VStylePainter::neutralFocusColor(const QColor& borderColor) {
    return multiplyAlpha(borderColor, 0.72);
}

bool VStylePainter::showsKeyboardFocus(Qt::FocusReason reason) noexcept {
...
QPainterPath VStylePainter::roundedRectPath(const QRectF& rect, qreal radius) {
    QPainterPath path;
    path.addRoundedRect(rect, std::max<qreal>(0.0, radius), std::max<qreal>(0.0, radius));
    return path;
}

void VStylePainter::drawRoundedPanel(QPainter& painter, const QRectF& rect, qreal radius,
...
void VStylePainter::drawFocusRing(QPainter& painter, const QRectF& rect, qreal radius,
...
void VStylePainter::drawHairline(QPainter& painter, const QPointF& start, const QPointF& end,
                                  const QColor& color) {
    painter.save();
    QPen pen(color, 0.0);
...
void VStylePainter::drawCheckmark(QPainter& painter, const QRectF& rect, const QColor& color,
                                   qreal width) {
    QPainterPath path;
...
void VStylePainter::drawChevron(QPainter& painter, const QRectF& rect, Qt::ArrowType direction,
                                 const QColor& color, qreal width) {
