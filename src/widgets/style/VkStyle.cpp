...
class VkComboPopupStyler final : public QObject {
  public:
    explicit VkComboPopupStyler(QObject* parent) : QObject(parent) {}

    ~VkComboPopupStyler() override {
    ...
    void polish(QWidget* popup) {
        ...
        // A QWidget mask is a binary region, so rounded edges are necessarily
        // aliased. A translucent top-level surface preserves the alpha coverage
        // generated by QPainter's antialiased path rasterizer.
        ...
        // backgrounds on Windows. QComboBox already uses this on most styles,
        // but setting it explicitly keeps the compositing contract stable.
...
void VkStyle::drawPrimitive(PrimitiveElement element, const QStyleOption* option, QPainter* painter,
    ...
    if (d->comboPopups->isPopupPart(widget) &&
        ...
        if (VkComboPopupStyler::isPopupContainer(widget)) {
            VkStylePainter::drawRoundedPanel(*painter, QRectF(option->rect),
                                             metrics.cornerRadiusLarge, colors.elevatedBackground,
    ...
    case PE_FrameFocusRect:
        ...
        return;
    case PE_PanelMenu:
        VkStylePainter::drawRoundedPanel(*painter, QRectF(option->rect), metrics.cornerRadiusLarge,
