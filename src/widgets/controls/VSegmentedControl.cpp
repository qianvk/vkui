...
#include "private/VSegmentedControl_p.h"
  ...
  protected:
    ...
    void paintEvent(QPaintEvent* event) override {
        ...
        if (m_pressProgress > 0.0) {
            ...
            const QColor overlay = VStylePainter::multiplyAlpha(pressFill, m_pressProgress * 0.82);
        ...
        QColor foreground =
            isChecked() ? VStylePainter::contrastingText(colors.accent) : colors.textPrimary;
        if (!isEnabled()) {
            foreground =
                isChecked() ? VStylePainter::multiplyAlpha(foreground, 0.56) : colors.textDisabled;
        }
        if (!iconRect.isEmpty()) {
            painter.save();
        ...
        if (hasFocus() && m_keyboardFocusVisible) {
            VStylePainter::drawFocusRing(
                ...
                QRectF(rect()).adjusted(metrics.spacing2, metrics.spacing2, -metrics.spacing2,
                                        -metrics.spacing2),
                metrics.cornerRadiusSmall, VStylePainter::neutralFocusColor(colors.borderStrong),
...
VSegmentedControlPrivate::VSegmentedControlPrivate(VSegmentedControl* owner)
...
bool VSegmentedControlPrivate::validIndex(int index) const noexcept {
    return index >= 0 && index < buttons.size();
}

void VSegmentedControlPrivate::updateButtonIds() {
    for (int index = 0; index < buttons.size(); ++index) {
        group->setId(buttons[index], index);
    }
}

void VSegmentedControlPrivate::relayout(bool animateIndicator) {
    if (buttons.isEmpty()) {
...
void VSegmentedControlPrivate::updateIndicator(bool animated) {
    ...
    indicatorAnimation->start(
        0.0, 1.0, VkMotionRole::StateTransition, [this, start, target](qreal progress) {
            indicatorRect =
                QRectF(start.topLeft() + (target.topLeft() - start.topLeft()) * progress,
                       start.size() + (target.size() - start.size()) * progress);
            q->update();
        });
}

void VSegmentedControlPrivate::moveSelection(int visualDelta, bool activate) {
...
int VSegmentedControlPrivate::nextEnabledIndex(int start, int logicalDelta) const {
...
void VSegmentedControlPrivate::updateFocusProxy() {
...
bool VSegmentedControlPrivate::eventFilter(QObject* watched, QEvent* event) {
...
VSegmentedControl::VSegmentedControl(QWidget* parent)
    ...
    connect(VkThemeManager::instance(), &VkThemeManager::themeChanged, this,
            [this](quint64, const VkThemeChanges changes) {
                ...
                if (changes.testFlag(VkThemeChange::Colors) || geometryChanged) {
...
VSegmentedControl::~VSegmentedControl() = default;

int VSegmentedControl::count() const noexcept {
    return static_cast<int>(d->buttons.size());
}

int VSegmentedControl::currentIndex() const noexcept {
    return d->currentIndex;
}

int VSegmentedControl::addSegment(const QString& text) {
    return addSegment(QIcon{}, text);
}

int VSegmentedControl::addSegment(const QIcon& icon, const QString& text) {
    return insertSegment(static_cast<int>(d->buttons.size()), icon, text);
}

int VSegmentedControl::insertSegment(int index, const QIcon& icon, const QString& text) {
...
void VSegmentedControl::removeSegment(int index) {
...
QString VSegmentedControl::segmentText(int index) const {
    return d->validIndex(index) ? d->buttons[index]->text() : QString{};
}

void VSegmentedControl::setSegmentText(int index, const QString& text) {
...
QIcon VSegmentedControl::segmentIcon(int index) const {
    return d->validIndex(index) ? d->buttons[index]->icon() : QIcon{};
}

void VSegmentedControl::setSegmentIcon(int index, const QIcon& icon) {
...
bool VSegmentedControl::isSegmentEnabled(int index) const {
    return d->validIndex(index) && d->buttons[index]->isEnabled();
}

void VSegmentedControl::setSegmentEnabled(int index, bool enabled) {
...
QString VSegmentedControl::segmentAccessibleName(int index) const {
    return d->validIndex(index) ? d->buttons[index]->accessibleName() : QString{};
}

void VSegmentedControl::setSegmentAccessibleName(int index, const QString& name) {
    if (!d->validIndex(index)) {
        return;
    }
...
QSize VSegmentedControl::sizeHint() const {
    if (d->buttons.isEmpty()) {
        const auto& metrics = VkThemeManager::instance()->theme().metrics();
...
void VSegmentedControl::paintEvent(QPaintEvent* event) {
    Q_UNUSED(event)
    ...
    VStylePainter::drawRoundedPanel(painter, QRectF(rect()), metrics.cornerRadiusRegular,
                                     isEnabled() ? colors.controlFill : colors.controlFillDisabled,
                                     colors.border, metrics.borderWidth);

    if (!d->indicatorRect.isEmpty()) {
        painter.fillPath(
            VStylePainter::roundedRectPath(d->indicatorRect, metrics.cornerRadiusSmall),
            colors.accent);
    }
    ...
    for (int index = 0; index + 1 < d->buttons.size(); ++index) {
        ...
        VStylePainter::drawHairline(painter, QPointF(x, rect().top() + metrics.spacing6),
                                     QPointF(x, rect().bottom() - metrics.spacing6),
                                     colors.separator);
    }
}

void VSegmentedControl::resizeEvent(QResizeEvent* event) {
    QWidget::resizeEvent(event);
    d->relayout(false);
}

void VSegmentedControl::changeEvent(QEvent* event) {
...
void VSegmentedControl::focusInEvent(QFocusEvent* event) {
...
void VSegmentedControl::keyPressEvent(QKeyEvent* event) {
