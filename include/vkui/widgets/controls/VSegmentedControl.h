...
class VSegmentedControlPrivate;

/** A connected, single-selection group of private checkable buttons. */
class VKUI_WIDGETS_EXPORT VSegmentedControl final : public QWidget {
    Q_OBJECT
    Q_PROPERTY(int currentIndex READ currentIndex WRITE setCurrentIndex NOTIFY currentIndexChanged)

  public:
    explicit VSegmentedControl(QWidget* parent = nullptr);
    ~VSegmentedControl() override;
  ...
  private:
    std::unique_ptr<VSegmentedControlPrivate> d;
};
